                ...
                <el-table-column
                    ...
                    <template v-slot="scope">
                        <slot v-bind:scope="{row: scope.row, col: col}">
                            <span v-if="!col.id2">{{ scope.row[col.id] }}</span>
                            <span v-else>{{ scope.row[col.id][col.id2] }}</span>
                        </slot>
                    </template>
                </el-table-column>
            </el-table>
...
export default {
    ...
    data() {
        return {
            leftTableData:JSON.parse(JSON.stringify(this.tableList)),
    ...
    methods: {
        ...
        handlePaginationCallBack() {
            if (this.showPagination) {
                ...
                let d = this.leftTableData.filter((item, index) => {
                    if (index >= (condition.pageIndex - 1) * condition.pageSize && index < condition.pageIndex * condition.pageSize) {
                        return true;
                    }
                    return false;
                });
                this.leftTableData = d
                this.totalSize = this.leftTableData.length
