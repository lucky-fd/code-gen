package com.yushi.code.tableCreator.helper;
...
import com.yushi.code.tableCreator.executor.TableExporter;
import com.yushi.code.tableCreator.function.FieldFunction;
...
import com.yushi.code.tableCreator.util.CollectionUtils;
...
public class EntityHelper {
  ...
  public static <T> void initEntity(final Class<T> clazz) {
    ...
    // 表定义更新
    final Configuration.DdlAuto ddlAuto = windContext.getConfiguration().getDdlAuto();
    if (ddlAuto == null || Configuration.DdlAuto.NONE.equals(ddlAuto)) {
      return;
    }
    if (!entityInfo.isMapToTable()) {
      return;
    }
    if (Configuration.DdlAuto.CREATE.equals(ddlAuto)) {
      TableExporter.of(windContext).createTable(entityInfo, true);
    } else if (Configuration.DdlAuto.UPDATE.equals(ddlAuto)) {
