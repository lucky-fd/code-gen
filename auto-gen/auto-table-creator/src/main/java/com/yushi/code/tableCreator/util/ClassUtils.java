...
public abstract class ClassUtils {
  ...
  /** 获取基本类型对应的包装类型. */
  @Nonnull
  public static Class<?> resolveWrapperClass(@Nonnull final Class<?> primitiveClass) {
    if (!primitiveClass.isPrimitive()) {
      return primitiveClass;
    }
    for (final Entry<Class<?>, Class<?>> entry : primitiveWrapperTypeMap.entrySet()) {
      if (entry.getValue().equals(primitiveClass)) {
        return entry.getKey();
      }
    }
    return primitiveClass;
