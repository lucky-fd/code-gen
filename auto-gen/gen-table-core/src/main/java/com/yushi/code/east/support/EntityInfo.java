...
import com.yushi.code.east.annotation.CreateTimestamp;
...
public final class EntityInfo {
  ...
  public static EntityInfo of(@Nonnull final EastConfiguration eastConfiguration) {
    return of(eastConfiguration, null);
  }

  public static EntityInfo of(@Nonnull final EastConfiguration eastConfiguration, Dialect dialect) {
  ...
  public static EntityInfo of(
          @Nonnull final Class<?> clazz, final EastConfiguration eastConfiguration, Dialect dialect) {
    EntityInfo entityInfo = new EntityInfo();
    ...
    entityInfo.setName(EntityUtils.getTableName(eastConfiguration, clazz));
    if (annotation == null || annotation.idGenerator().equals(IdGenerator.VoidIdGenerator.class)) {
      entityInfo.setIdGenerator(eastConfiguration.getIdGenerator());
    } else {
    ...
    List<EntityIndex> entityIndexes =
        EntityIndex.getEntityIndexes(clazz, entityInfo.getName(), new HashSet<>(entityColumns), dialect);
